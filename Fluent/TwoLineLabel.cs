...
namespace Fluent
{
    ...
    public class TwoLineLabel: Control
    {
        ...
        public static readonly DependencyProperty TextProperty =
            DependencyProperty.Register("Text", typeof(object), typeof(TwoLineLabel), new UIPropertyMetadata("TwoLineLabel", OnTextChanged));
        ...
        void UpdateTextRun()
        {
            ...
            {
                string textContent = Text is string ? (string)Text : ((Label)Text).Content as string;
                textRun.Text = 
                textRun2.Text = "";
                string text = textContent.Trim();
                if (HasTwoLines)
                {
                    int centerIndex = textContent.Length / 2;
